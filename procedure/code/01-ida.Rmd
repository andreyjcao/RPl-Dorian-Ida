...
```{r bind-original}
ida_raw <- rbind(tevent_raw1, tevent_raw2, tevent_raw3, tevent_raw4) #union operation, table union, set union
ida_raw_new <- ida_raw %>% distinct(status_id, .keep_all = TRUE)
```

```{r union-original}
tevent_raw1and2 <- union(tevent_raw1, tevent_raw2)
tevent_raw1and2and3 <- union(tevent_raw1and2, tevent_raw3)
ida_raw <- union(tevent_raw1and2and3, tevent_raw4)
```
...
```{r network}
ida_network <- network_graph(ida, c("quote")) #who is quoting who, graphing tweets from a city
plot.igraph(ida_network)
```
...
These state codes represent (). Hurricane Ida made contact with these states.
...
counties <- counties %>%
  mutate(dorrate = dorian / POPESTIMATE * 10000) %>% # dorrate is tweets per 10,000
